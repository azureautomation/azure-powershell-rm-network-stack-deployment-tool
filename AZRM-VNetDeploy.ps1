...
Vnet Resource Group
.PARAMETER vnet2rg
2nd Vnet Resource Groop (peering)
...
.PARAMETER VNetName2
Name of new or existing 2nd vnet to deploy peering too.
.PARAMETER VnetPeering
Deploys Vnet-Vnet peering
.PARAMETER CreateNSG
Creates a New NSG
.PARAMETER CreateExtLoadBalancer
Creats a new external load balancer.
.PARAMETER CreateIntLoadBalancer
Creats a new internal load balancer.
.PARAMETER LBType
Load Balancer Type
.PARAMETER IntLBName
Load Balancer Name
...
.EXAMPLE
\.AZRM-VnetDeploy.ps1 -vnetrg ResGroup -addvnet -vnet VNET
.EXAMPLE
\.AZRM-VnetDeploy.ps1 -vnetrg ResGroup -vnet VNET -creatensg -nsgname nsg
.EXAMPLE
\.AZRM-VnetDeploy.ps1 -vnetrg ResGroup -vnet VNET -createintloadbalancer -intlbname intlb -lbsubnet 4 -LBPvtIP 10.20.4.10
.EXAMPLE
\.AZRM-VnetDeploy.ps1 -vnetrg ResGroup -vnet VNET -createextloadbalancer -extlbname extlb
.EXAMPLE
...
[Parameter(Mandatory=$False,ValueFromPipelinebyPropertyName=$true)]
[string]
$SubnetNameAddPrefix2 = 'internallan',
[Parameter(Mandatory=$False,ValueFromPipelinebyPropertyName=$true)]
[string]
$Profile = "profile",
[Parameter(Mandatory=$False,ValueFromPipelinebyPropertyName=$true)]
...
$fileexist = Test-Path $RemoveScriptFileName
  if($fileexist)
  {
		Write-Host "Remove Script Found"
  }
  else
  {
  Write-Host "Saved Profile to $ProfileFile"
...
Write-ConfigVNet
	...
	Try
	{
	New-AzureRmVirtualNetwork -Location $Location -Name $VNetName -ResourceGroupName $vnetrg -AddressPrefix $AddRange -Subnet $subnet1,$subnet2 –Confirm:$false -WarningAction SilentlyContinue -Force | Out-Null
